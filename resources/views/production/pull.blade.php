...
@section('table-body')
    @foreach($pulls as $index => $pull)
        <tr data-id="{{ $pull->id }}">
            ...
            <td>{{ $index + 1 }}</td>
            <td>{{ $pull->history->identifier->kode }}</td>
...
@section('custom-js')
    $(document).on('click', '.btnEdit', function() {
        ...
        fetch(`/pulls/${id}`)
            ...
            .then(pull => {
                $('#item_id').val(pull.id);
                $('#histories_id').val(pull.histories_id);
                $('#employees_id').val(pull.employees_id);
                $('#tgl_mulai').val(pull.tgl_mulai);
                $('#biji_masuk').val(pull.biji_masuk);
                $('#berat_masuk').val(pull.berat_masuk);
