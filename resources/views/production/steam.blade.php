        ...
        @foreach($steams as $index => $steam)
            <tr data-id="{{ $steam->id }}">
                ...
                <td>
                    ...
                    <a href="{{ route('steams.export', $steam->id) }}" class="btn btn-sm btn-primary" target="_blank">Cetak Form</a>
                </td>
            </tr>
        @endforeach
    @stop
        ...
        for (let i = 1; i <= jumlah; i++) {
            html += `
                <div class="border rounded p-3 mb-3">
                    ...
                    <input type="number" class="form-control mb-2 jml-tray" data-index="${i}" step="1" min="0" max="6">
                </div>
            `;
        }
        ...
        $('#btnSubmitAll').off().on('click', function () {
            ...
            for (let i = 1; i <= jumlah; i++) {
                list.push({
                    ...
                    jml_tray: $(`.jml-tray[data-index="${i}"]`).val(),
                });
            }
                ...
                .then(result => {
                    ...
                    $('#btnSubmitAll').off().on('click', function () {
                        ...
                        let payload = {
                            ...
                            waktu: $('#edit_waktu').val(),
                            suhu_total: parseFloat($('#edit_suhu_total').val()),
                            waktu_total: $('#edit_waktu_total').val(),
                            keterangan: $('#edit_keterangan').val(),
                            jml_tray: $('#edit_jml_tray').val(),
                        };
