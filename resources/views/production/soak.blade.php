    ...
    @foreach($soaks as $index => $soak)
        <tr data-id="{{ $soak->id }}">
            ...
            <td>
                ...
                <a href="{{ route('soaks.export', $soak->id) }}" class="btn btn-sm btn-primary" target="_blank">Cetak Form</a>
            </td>
        </tr>
    @endforeach
@stop
...
@section('custom-js')
    ...
    $(document).on('click', '.btnEdit', function() {
        ...
        fetch(`/soaks/${id}`)
            ...
            .then(soak => {
                $('#item_id').val(soak.id);
                $('#histories_id').val(soak.histories_id).trigger('change');
                $('#employees_id').val(soak.employees_id);
                $('#tanggal').val(soak.tanggal);
                $('#biji').val(soak.biji);
